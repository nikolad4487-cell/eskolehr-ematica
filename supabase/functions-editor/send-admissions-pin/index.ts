...
async function resolveSchoolAdministratorPhone(
  ...
  const { data: roles, error: rolesError } = await admin.from('user_school_roles').select('user_id, role, status').eq('school_id', schoolId);
  ...
  const activeRoles = (roles ?? [])
    ...
    .sort((a, b) => priority.indexOf(String(a.role).toUpperCase()) - priority.indexOf(String(b.role).toUpperCase()));
  for (const role of activeRoles) {
    const { data: profile, error } = await admin.from('user_profiles').select('*').eq('id', role.user_id).maybeSingle();
    if (error) throw error;
    const phone = normalizePhone(String(profile?.mobile ?? profile?.phone ?? profile?.phone_number ?? ''));
...
Deno.serve(async (req) => {
  ...
  try {
    const { track, phone_number: phoneNumber, delivery_method: requestedDeliveryMethod } = await req.json() as {
    ...
    const deliveryMethod: DeliveryMethod = requestedDeliveryMethod ?? existing?.delivery_method ?? 'STUDENT_PHONE';
    let phone = '';
    let deliveryAdminProfileId: string | null = null;
    if (deliveryMethod === 'SCHOOL_ADMIN_PHONE') {
      if (!context.candidate.source_school_id || !context.candidate.school_year_id) throw new Error('Kandidat nema povezanu školu ili školsku godinu.');
      const contact = await resolveSchoolAdministratorPhone(admin, context.candidate.source_school_id, context.candidate.school_year_id);
      phone = contact.phone;
      deliveryAdminProfileId = contact.profileId;
    } else {
      const enteredDigits = String(phoneNumber ?? '').replace(/\D/g, '').replace(/^385/, '');
      phone = enteredDigits.length === 9 ? `+385${enteredDigits}` : normalizePhone(String(existing?.phone ?? ''));
    }
